<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ACT International</title>
    <style>
    </style>
    <link rel="icon" type="image/x-icon" href="../images/Logo2.png">
    
</head>
<body>
    		<!-- This part Special To Configrutions -->
    <section id="emp">
        <div>
            <form method="post">
                <h2 class="text-center">New Employee Group</h2>
                <table>
                    <tr>
                        <td><label>Group Name</label></td>
                        <td><input class="input-lg form-control" name="groupemp" type="text" required/></td>
                    </tr>
                    <tr>
                        <td><label>Group Id</label></td>
                        <td><input class="input-lg form-control" name="groupid" type="text" required/></td>
                    </tr>
                    <tr>
                        <td><label>Group Number</label></td>
                        <td><input class="input-lg form-control" name="groupnum" type="text" required/></td>
                    </tr>
                </table>
                <button id="btn1" class="btn btn-danger">Close</button>
                <input id="btnAdd2" class="btn btn-danger" value="submit" type="submit"/>
			</form>
        </div>
    </section>

    <script>
    </script>
    </body>

</html>